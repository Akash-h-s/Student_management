...
const MarksEntrySystem: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // For now, hardcoded to 23 (your actual teacher ID)
    setTeacherId(23);
    
    // In production, you would fetch from auth context or token:
    // const token = localStorage.getItem('token');
    // const decoded = jwt_decode(token);
    // setTeacherId(decoded.teacher_id);
  }, []);
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          ...
          {teacherId && (
            <div className="mb-4 p-2 bg-blue-50 border border-blue-200 rounded text-sm text-blue-800">
              Logged in as Teacher ID: {teacherId}
            </div>
          )}
